...
Note: There's . and - in snow_cover
...
snowsurvey_fixed <- 
  snowsurvey_csv %>% 
  mutate(Snow_cover = ifelse(Snow_cover %in% c("-","."), NA, Snow_cover)) 
...
snowsurvey_fixed <- 
 snowsurvey_fixed %>% 
  mutate(Snow_cover = ifelse(Snow_cover %in% c("-","n/a","."), NA, Snow_cover))
...
count(snowsurvey_fixed, Snow_cover)
```
...
```
**Note**: Because we dont feel comfortable enough with how to interperate X, we will convert it to a precence/Absence 
...
species_long <- species_csv %>% 
  pivot_longer(
    ...
    values_transform = list(species_count = as.character) # convert to character to avoid losses due to conversion to numeric
  )
```
...
species_presence <- species_presence %>%
  ...
  mutate(species_presence = as.numeric(species_presence)) %>%
  arrange(Date)
